CREATE OR REPLACE TABLE `bi-fcom-drmb-local-pe-sbx.Dragonite_SX_KPIs.reporte_backlog` AS (
  ...
  final AS (
    SELECT
      ...
      gsc.src_id AS sellerId, --seller_ID
      INITCAP(`bi-fcom-drmb-local-pe-sbx.Dragonite_SX_KPIs`.FIX_STRING(gsc.name)) AS sellerName, --seller_Name
  ...
  SELECT
    ...
    CASE
      WHEN sellerId IN (
          ...
          FROM Reporte_BL
          GROUP BY sellerId
          HAVING COUNT(DISTINCT lastStatusGSC) = 2
      ) THEN 'Speech_2'
      WHEN sellerId IN (
          SELECT sellerId
          FROM Reporte_BL
          GROUP BY sellerId
          HAVING COUNT(DISTINCT lastStatusGSC) = 1
          AND SUM(CASE WHEN lastStatusGSC = 'ready_to_ship' THEN 1 ELSE 0 END) > 0
      ) THEN 'Speech_3'
      WHEN sellerId IN (
          SELECT sellerId
          FROM Reporte_BL
